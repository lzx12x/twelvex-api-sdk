...
import icu.twelvex.twelvexapisdk.exception.ErrorCode;
import icu.twelvex.twelvexapisdk.exception.ErrorResponse;
import icu.twelvex.twelvexapisdk.model.request.BaseRequest;
import icu.twelvex.twelvexapisdk.model.response.ResultResponse;
import icu.twelvex.twelvexapisdk.utils.SignUtils;
import lombok.Data;
...
public abstract class BaseService implements ApiService {
    private TwelveXApiClient twelvexApiClient;
    /**
     * 网关HOST
     */
//    private String gatewayHost = "https://localhost:8090/api";
     ...
     */
    public void checkConfig(TwelveXApiClient twelvexApiClient) throws ApiException {
        if (twelvexApiClient == null && this.getTwelvexApiClient() == null) {
            throw new ApiException(ErrorCode.NO_AUTH_ERROR, "请先配置密钥AccessKey/SecretKey");
        }
        if (twelvexApiClient != null && !StringUtils.isAnyBlank(twelvexApiClient.getAccessKey(), twelvexApiClient.getSecretKey())) {
            this.setTwelvexApiClient(twelvexApiClient);
        }
    }
    ...
    private <O, T extends ResultResponse> HttpRequest getHttpRequestByRequestMethod(BaseRequest<O, T> request) throws ApiException {
        ...
        return httpRequest.addHeaders(getHeaders(JSONUtil.toJsonStr(request), twelvexApiClient)).body(JSONUtil.toJsonStr(request.getRequestParams()));
    }
     ...
     */
    private Map<String, String> getHeaders(String body, TwelveXApiClient twelvexApiClient) {
        Map<String, String> hashMap = new HashMap<>(4);
        hashMap.put("accessKey", twelvexApiClient.getAccessKey());
        String encodedBody = SecureUtil.md5(body);
        hashMap.put("body", encodedBody);
        hashMap.put("timestamp", String.valueOf(System.currentTimeMillis() / 1000));
        hashMap.put("sign", SignUtils.getSign(encodedBody, twelvexApiClient.getSecretKey()));
        return hashMap;
    }
    ...
    @Override
    public <O, T extends ResultResponse> T request(TwelveXApiClient twelvexApiClient, BaseRequest<O, T> request) throws ApiException {
        checkConfig(twelvexApiClient);
